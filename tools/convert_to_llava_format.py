...
def build_image_mapping(image_dir: str) -> Dict[str, str]:
    ...
    if not image_dir_path.exists():
        print(f"⚠️  Warning: Image directory does not exist: {image_dir}")
    ...
    print(f"✅ Built mapping for {len(image_mapping):,} unique image IDs")
    return image_mapping
...
def print_statistics(stats: Dict[str, Any]):
    ...
    print(f"🔧 Records with metadata: {stats['records_with_meta']:,}")
    print(f"💬 Records with prompts: {stats['records_with_prompt']:,}")
    print(f"✅ Valid prompts for training: {stats['valid_prompts']:,}")
...
def main():
    ...
    print(f"📂 Loading consolidated JSON from: {input_path}")
    ...
    if not isinstance(records, list):
        print("❌ Input file should contain a JSON array")
        return
    
    print(f"📊 Loaded {len(records):,} records")
    
    # Build image mapping from the specified directory
    print(f"\n🖼️  Building image mapping...")
    image_mapping = build_image_mapping(args.image_dir)
    
    if not image_mapping:
        print("⚠️  Warning: No images found in the specified directory!")
        print("   The output will be empty unless images are available.")
    
    # Limit records if specified
    if args.max_records:
        records = records[:args.max_records]
        print(f"🔢 Limited to {len(records):,} records for processing")
    ...
    if args.stats_only:
        print("\n✅ Statistics analysis complete (stats-only mode)")
        return
    ...
    if not llava_records:
        print("❌ No valid records found for conversion")
        return
    ...
    print(f"\n💾 Saving {len(llava_records):,} LLaVA records to: {output_path}")
    ...
    print(f"📊 Records converted: {len(llava_records):,} / {len(records):,}")
    print(f"📈 Success rate: {conversion_rate:.2f}%")
    print(f"💾 File size: {file_size_mb:.2f} MB")
    print(f"🖼️  Image directory: {args.image_dir}")
    print(f"📸 Total images available: {len(image_mapping):,}")
