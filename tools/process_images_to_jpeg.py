...
1. Convert all images to JPEG format with quality 90
2. If minimum dimension > 512, resize so minimum dimension = 512 (preserving aspect ratio)
3. Use different interpolation algorithms for resizing (LANCZOS, BICUBIC, BILINEAR)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Process images: convert to JPEG and resize if needed"
    )
    parser.add_argument(
        "--meta-json",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=512,
        help="Target minimum dimension for resizing (default: 512)",
    )
    parser.add_argument(
        "--jpeg-quality",
        type=int,
...
def process_single_image(args_tuple):
    ...
    if preserve_structure:
        ...
        dst_path = Path(output_root) / rel_path
    else:
        # Flatten structure: use image filename only
        filename = Path(record['image_path']).stem + '.jpg'
        dst_path = Path(output_root) / filename
    ...
    try:
        ...
        with Image.open(src_path) as img:
            ...
            # Resize if minimum dimension > min_size
            if min_dim > min_size:
        ...
        return True, str(src_path), str(dst_path), resize_info
...
def main():
    ...
    print(f"  Resize methods:   Random from {list(RESIZE_METHODS.keys())}")
    ...
    if args.preserve_structure and success_count > 0:
        print(f"\nNote: Original metadata structure preserved.")
        print(f"You can create a new metadata JSON pointing to {args.output_root}")
