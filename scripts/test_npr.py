#!/usr/bin/env python3
"""Test NPR Binary Classifier.

This script evaluates a trained NPR model on the test set and provides
detailed performance metrics for real/fake detection.

Usage examples:
    python scripts/test_npr.py --config configs/npr_train_config.yaml \
        --checkpoint ./checkpoints/npr_binary/best_model.pt

    python scripts/test_npr.py --config configs/npr_train_config.yaml \
        --checkpoint ./checkpoints/npr_binary/checkpoint_epoch_29.pt \
        --save_predictions
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Test NPR Binary Classifier"
    )
    parser.add_argument(
        "--config",
        type=str,
...
def test_npr(
    ...
    """Test NPR model and compute detailed metrics."""
    ...
    if save_predictions:
        ...
        plt.title('NPR Confusion Matrix')
        plt.colorbar()
...
def main():
    ...
    print("Creating NPR model...")
    model = create_npr(config, verbose=True)
