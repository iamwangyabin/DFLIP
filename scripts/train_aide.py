#!/usr/bin/env python3
"""Train AIDE Binary Classifier with single-node multi-GPU (DDP).

This script trains an AIDE binary classifier for real/fake detection only.
AIDE combines high-pass filtering, ResNet features, and ConvNeXt features.

Usage examples (single node with 4 GPUs):

    torchrun --nproc_per_node=4 scripts/train_aide.py \
        --config configs/aide_train_config.yaml

    # Or single GPU:
    python scripts/train_aide.py --config configs/aide_train_config.yaml
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train AIDE Binary Classifier (DDP)"
    )
    parser.add_argument(
        "--config",
        type=str,
        default="configs/aide_train_config.yaml",
...
def evaluate_aide(
    ...
    """Evaluate AIDE on validation set."""
    from tqdm import tqdm
    import torch.distributed as dist
...
def main():
    ...
    if is_main_process:
        print("Creating AIDE binary classifier model...")
    
    model = create_aide(config, verbose=is_main_process)
    ...
    if use_ddp:
        ...
        model = DDP(
            ...
            find_unused_parameters=True  # AIDE has frozen ConvNeXt components
        )
    ...
    for epoch in range(start_epoch, train_config["num_epochs"]):
        ...
        # Training
        train_metrics = train_epoch_aide(
            model,
            train_loader,
            optimizer,
            scheduler,
