#!/usr/bin/env python3
"""Train RINE Binary Classifier with single-node multi-GPU (DDP).

This script trains a RINE binary classifier for real/fake detection only.
This is a comparison method using CLIP intermediate features with attention mechanism for fake detection.

Usage examples (single node with 4 GPUs):

    torchrun --nproc_per_node=4 scripts/train_rine.py \
        --config configs/rine_train_config.yaml

    # Or single GPU:
    python scripts/train_rine.py --config configs/rine_train_config.yaml
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train RINE Binary Classifier (DDP)"
    )
    parser.add_argument(
        "--config",
        type=str,
        default="configs/rine_train_config.yaml",
...
def evaluate_rine(
    ...
    """Evaluate RINE on validation set."""
    from tqdm import tqdm
    import torch.distributed as dist
...
def main():
    ...
    if is_main_process:
        print("Creating RINE binary classifier model...")
    
    model = create_rine(config, verbose=is_main_process)
    ...
    for epoch in range(start_epoch, train_config["num_epochs"]):
        ...
        # Training
        train_metrics = train_epoch_rine(
            model,
            train_loader,
            optimizer,
            scheduler,
