#!/usr/bin/env python3
"""Train CNNDet Binary Classifier with single-node multi-GPU (DDP).

This script trains a CNNDet binary classifier for real/fake detection only.
This is a classic baseline method for comparison with multi-task approaches.

Usage examples (single node with 4 GPUs):

    torchrun --nproc_per_node=4 scripts/train_cnndet.py \
        --config configs/cnndet_train_config.yaml

    # Or single GPU:
    python scripts/train_cnndet.py --config configs/cnndet_train_config.yaml
"""
...
from models.cnndet import create_cnndet  # noqa: E402
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train CNNDet Binary Classifier (DDP)"
    )
    parser.add_argument(
        "--config",
        type=str,
        default="configs/cnndet_train_config.yaml",
...
def train_epoch_cnndet(
    ...
    """Train CNNDet for a single epoch."""
...
def evaluate_cnndet(
    ...
    """Evaluate CNNDet on validation set."""
    from tqdm import tqdm
    import torch.distributed as dist
...
def main():
    ...
    if is_main_process:
        print("Creating CNNDet binary classifier model...")
    
    model = create_cnndet(config, verbose=is_main_process)
    ...
    for epoch in range(start_epoch, train_config["num_epochs"]):
        ...
        # Training
        train_metrics = train_epoch_cnndet(
            model,
            train_loader,
            optimizer,
            scheduler,
