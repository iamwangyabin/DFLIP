#!/usr/bin/env python3
"""Train NPR Binary Classifier with single-node multi-GPU (DDP).

This script trains an NPR binary classifier for real/fake detection only.
This is a comparison method using Noise Pattern Residual for fake detection.

Usage examples (single node with 4 GPUs):

    torchrun --nproc_per_node=4 scripts/train_npr.py \
        --config configs/npr_train_config.yaml

    # Or single GPU:
    python scripts/train_npr.py --config configs/npr_train_config.yaml
"""
...
from models.npr import create_npr  # noqa: E402
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train NPR Binary Classifier (DDP)"
    )
    parser.add_argument(
        "--config",
        type=str,
        default="configs/npr_train_config.yaml",
...
def evaluate_npr(
    ...
    """Evaluate NPR on validation set."""
    from tqdm import tqdm
    import torch.distributed as dist
...
def main():
    ...
    if is_main_process:
        print("Creating NPR binary classifier model...")
    
    model = create_npr(config, verbose=is_main_process)
